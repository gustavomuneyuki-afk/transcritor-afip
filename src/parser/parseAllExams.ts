...
import { parseCardiacMuscle } from "./cardiacMuscleParser";
import { parseCoagulation } from "./coagulationParser";
...
import { parseImmunology } from "./immunologyParser";
...
import { parseUrinalysis } from "./urinalysisParser";
import { parseUrineProtein } from "./urineProteinParser";
import { parseTumorMarkers } from "./tumorMarkerParser";

export function parseAllExams(lines: PdfLine[]): ExamResult {
  return {
    ...
    vitaminMetabolism: parseVitaminMetabolism(lines),
    immunology: parseImmunology(lines),
    cardiacMuscle: parseCardiacMuscle(lines),
    coagulation: parseCoagulation(lines),
    urinalysis: parseUrinalysis(lines),
    urineProtein: parseUrineProtein(lines),
    tumorMarkers: parseTumorMarkers(lines),
  };
}
